...
struct Pseudo {
    ...
    Pseudo operator+(Pseudo o) const {
        return Pseudo(s+o.s);
    }
    ...
    Scalar operator*(Pseudo o) const {
        return Scalar(s*o.s);
    }
...
inline Point operator*(Scalar s, Point const& p) {
    return p*s;
}
...
inline Plane operator*(Scalar s, Plane const& p) {
    return p*s;
}
...
inline Line operator*(Scalar s, Line const& l) {
    return l*s;
}
...
inline Point operator*(Line const& b, Plane const& a) {
	return Point( 	 
		 a.b*b.i + a.c*b.j + a.d*b.m,
		-a.c*b.k - a.d*b.l - a.a*b.i,
		 a.d*b.n - a.a*b.j + a.b*b.k,
		-a.a*b.m + a.b*b.l - a.c*b.n );
}
...
struct Transform {
	...
	Transform(
        ...
        : det(i1*i2*i3*i0)
        , idet(1.f/det.s)
		// this is the matrix inverse, faster to negate the determinant than to
		// reorder the points (compiler can find more common subexpressions)
		, m0(i1*i2*i3*idet)
		, m1(i0*i2*i3*-idet) 
		, m2(i0*i1*i3*idet)
		, m3(i0*i1*i2*-idet)
	...
	static Transform  RotateX(float t) {
		return {Point(1,0,0,0),
                Point(0,cos(t),sin(t),0),
                Point(0,-sin(t),cos(t),0),
                Point(0,0,0,1)};
	}
	
	static Transform  RotateY(float t) {
		return {Point(cos(t),0,-sin(t),0),
                Point(0,1,0,0),
                Point(sin(t),0,cos(t),0),
                Point(0,0,0,1)};
	}
	
	static Transform  RotateZ(float t) {
		return {Point(cos(t),sin(t),0,0),
                Point(-sin(t),cos(t),0,0),
                Point(0,0,1,0),
                Point(0,0,0,1)};
	}
...
inline Point operator*(Transform const& t, Point const& o) {
	return Point(float(t.m0*o),float(t.m1*o),float(t.m2*o),float(t.m3*o));
}

// plane * matrix
inline Plane operator*(Plane const& p, Transform const& t) {
	return Plane(float(p*t.i0),float(p*t.i1),float(p*t.i2),float(p*t.i3));
}
...
struct P2 {
    ...
    P2 operator*(Scalar f) const {
        return P2(u*f.s, v*f.s);
    }
...
struct V3 {
    ...
    V3 operator*(Scalar f) const {
        return V3(x*f.s, y*f.s, z*f.s);
    }
...
inline Transform Transform::Rotate(V3 const& axis, float t) {
    ...
    return Transform(
        Point(x*x+(1-x*x)*c, y*x*(1-c)+z*s, z*x*(1-c)-y*s, 0),
        Point(x*y*(1-c)-z*s, y*y+(1-y*y)*c, z*y*(1-c)+x*s, 0),
        Point(x*z*(1-c)+y*s, y*z*(1-c)-x*s, z*z+(1-z*z)*c, 0),
        Point(0,0,0,1)
        );
}
