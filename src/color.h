...
struct rgba {
    ...
    rgba operator*(double s) const { return rgba(r * s, g * s, b * s, a * s); }
...
static const double RGBtoXYZ[3][3] = {{+0.5149f, +0.3244f, +0.1607f},
...
struct Spectrum {
    ...
    Spectrum(double s) {
        for (unsigned int i = 0; i < longestWavelength - shortestWavelength;
             i++)
            spectrum[i] = s;
    }

    Spectrum(const rgba& o) {
        // A very simple conversion from an rgb triple to a spectral
        // distribution. For a better version, see Smit ?
    ...
    operator rgba() const {
        ...
        return rgba(
            x * XYZtoRGB[0][0] + y * XYZtoRGB[0][1] + z * XYZtoRGB[0][2],
            x * XYZtoRGB[1][0] + y * XYZtoRGB[1][1] + z * XYZtoRGB[1][2],
            x * XYZtoRGB[2][0] + y * XYZtoRGB[2][1] + z * XYZtoRGB[2][2], 1);
    }
    ...
    Spectrum operator+(Spectrum const& o) const {
        ...
        for (unsigned int i = 0; i < longestWavelength - shortestWavelength;
             i++)
            r.spectrum[i] = spectrum[i] * o.spectrum[i];
        return r;
    }

    double operator[](double wavelength) const {
        if (wavelength < shortestWavelength || wavelength > longestWavelength)
            return 0;
        else
            return spectrum[(unsigned int)wavelength - shortestWavelength];
    }
    ...
    static rgba torgb(double wavelength, float weight) {
        ...
        /*printf("%f %f %f\n", x, y, z);
        rgba rr = rgba(
        weight*(x*XYZtoRGB[0][0]+y*XYZtoRGB[0][1]+z*XYZtoRGB[0][2]),
