...
//	RGB <--> XYZ matrices
static const double XYZtoRGB[3][3] = 
	{ { +2.5623f, -1.1661f, -0.3962f },
	  { -1.0215f, +1.9778f, +0.0437f },
	  { +0.0752f, -0.2562f, +1.1810f } };

static const double RGBtoXYZ[3][3] =
	{ { +0.5149f, +0.3244f, +0.1607f },
	  { +0.2654f, +0.6704f, +0.0642f },
	  { +0.0248f, +0.1248f, +0.8504f } };

//
//	Scale factor for tristimulus curves
//
static const double SpectrumToXYZ_N = 106.85556f;

/**
 * The tristimulus curves.  Represented as series of triplets for the X, Y,
 * and Z responses from 375nm to 800nm at 1nm intervals.
 */	
	static const double SpectrumToXYZ[ 425 ][ 3 ] = {
...
struct Spectrum {
	...
	Spectrum( double s ) {
		for(unsigned int i = 0; i < longestWavelength-shortestWavelength; i++) spectrum[i] = s;
	}

	Spectrum( const rgba& o ) {
		// A very simple conversion from an rgb triple to a spectral distribution. For a better version, see Smit ?
		for(unsigned int i = 375; i < 400; i++)  spectrum[i-375] = 0;
		for(unsigned int i = 400; i < 500; i++) spectrum[i-400] = o.b;
		for(unsigned int i = 500; i < 600; i++) spectrum[i-500] = o.g;
		for(unsigned int i = 600; i < 700; i++) spectrum[i-600] = o.r;
		for(unsigned int i = 700; i < 800; i++) spectrum[i-700] = 0;
	}
	
	operator rgba() const {
		double x = 0, y = 0, z = 0;

		for( unsigned int i = 0; i < longestWavelength-shortestWavelength; i++ )
		{
			x += spectrum[i]*SpectrumToXYZ[i][0];
			y += spectrum[i]*SpectrumToXYZ[i][1];
			z += spectrum[i]*SpectrumToXYZ[i][2];				
		}
		...
		return rgba(	x*XYZtoRGB[0][0]+y*XYZtoRGB[0][1]+z*XYZtoRGB[0][2],
	...
	Spectrum operator*(double s) const {
		Spectrum r;
		for(unsigned int i = 0; i < longestWavelength-shortestWavelength; i++) r.spectrum[i] = spectrum[i]*s;
		return r;
	}

	Spectrum operator+(Spectrum const& o) const {
		Spectrum r;
		for(unsigned int i = 0; i < longestWavelength-shortestWavelength; i++) r.spectrum[i] = spectrum[i]*o.spectrum[i];
		return r;
	}

	double operator[](double wavelength) const {
		if(wavelength < shortestWavelength || wavelength > longestWavelength) return 0;
		else return spectrum[(unsigned int)wavelength-shortestWavelength];
	}

	static rgba torgb(double wavelength, float weight) {
		if(wavelength < shortestWavelength || wavelength > longestWavelength) return rgba(0,0,0,0);
		unsigned int i = (unsigned int)wavelength-shortestWavelength;
		double x = SpectrumToXYZ[i][0]/SpectrumToXYZ_N;
		double y = SpectrumToXYZ[i][1]/SpectrumToXYZ_N;
		double z = SpectrumToXYZ[i][2]/SpectrumToXYZ_N;
		/*printf("%f %f %f\n", x, y, z); 
		rgba rr = rgba(	weight*(x*XYZtoRGB[0][0]+y*XYZtoRGB[0][1]+z*XYZtoRGB[0][2]),
				weight*(x*XYZtoRGB[1][0]+y*XYZtoRGB[1][1]+z*XYZtoRGB[1][2]),
				weight*(x*XYZtoRGB[2][0]+y*XYZtoRGB[2][1]+z*XYZtoRGB[2][2]),
				1);
		printf("%f %f %f\n", rr.r, rr.g, rr.b);
		return rr;*/
	...
	static const unsigned int shortestWavelength=375;
	static const unsigned int longestWavelength=800;
