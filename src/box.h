...
struct BVH {
    ...
    struct State {
        ...
        std::vector<P3> c;
        std::vector<int> temp;
        State(std::vector<Box> const& b, std::vector<P3> const& c)
            : b(b), c(c), temp(b.size()) {}
    };

    Box const& root() const { return n[0].box; }
    ...
    void construct(State& state, int nindex, Box const& vb, Box const& cb,
                   int begin, int end) {
        int d;
        ...
        float k1 = (K * 0.9999) /
                   (d == 0 ? cb.n.x - cb.m.x
                           : (d == 1 ? cb.n.y - cb.m.y : cb.n.z - cb.m.z));

        for (int i = begin; i < end; ++i) {
            int bin = k1 * (d == 0 ? state.c[o[i]].x - cb.m.x
        ...
        for (int i = begin; i < end; ++i) {
            ...
            if (bin < best) {
                state.temp[bindex++] = o[i];
            } else {
                state.temp[eindex--] = o[i];
            }
        }
        for (int i = begin; i < end; ++i) {
            o[i] = state.temp[i];
        ...
        // push back child pair...area seems to be a good ordering (better than
        // ray direction?)
