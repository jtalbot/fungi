...
ImageTexture::Impl::Impl(std::string const& filename)
{
    readPNG(filename);
}
...
rgba ImageTexture::eval(P2 const& uv) const
{
    return a->eval(uv);
}
...
std::pair<rgba, V3> DiffuseMaterial::sample(Dip const& dip, V3 const& in) const
{
