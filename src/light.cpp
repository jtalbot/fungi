...
void InfiniteAreaLight::buildDistribution()
{
    ...
    for(int y = 0; y < height; ++y)
    {
        ...
        for(int x = 0; x < width; ++x)
        {
            ...
            auto color = rgba(data[index+0], data[index+1], data[index+2], 1);
...
{
    // sample row
    auto v = std::lower_bound(cdf_rows.begin(), cdf_rows.end(), cdf_rows.back()*gi_random()) - cdf_rows.begin();
    auto const& row = cdf_columns.at(v);
    
    // sample column
    auto u = std::lower_bound(row.begin(), row.end(), row.back()*gi_random()) - row.begin();
