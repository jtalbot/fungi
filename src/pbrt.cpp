...
struct Params
{
    ...
    bool hasFloat(std::string const& key) const {
        return reals.count(key) > 0;
    }
    ...
    bool hasRGB(std::string const& key) const {
        return reals.count(key) > 0;
    }
...
void tSet(const Transform& t) {
    ...
    if(top.active & ActiveTransform::StartTime)
        top.start = t;
    if(top.active & ActiveTransform::EndTime)
        top.end = t;
}
...
void pbrtInit(std::string path) {
    ...
    PBRTState::transforms.push(PBRTState::TransformState{
        0,1,Transform::Identity(), Transform::Identity(), PBRTState::ActiveTransform::All});
    ...
    PBRTState::materialStack.push(std::make_shared<DiffuseMaterial>(std::make_shared<ConstantTexture>(rgba(0,0,0,0))));
...
void pbrtIdentity() {
    PBRTState::tSet(Transform::Identity());
}
...
void pbrtTransform(float t[16]) {
    PBRTState::tSet(Transform(
        Point(t[0],t[4],t[8],t[12]),
        Point(t[1],t[5],t[9],t[13]),
        Point(t[2],t[6],t[10],t[14]),
        Point(t[3],t[7],t[11],t[15])));
}
...
void pbrtLookAt(float ex, float ey, float ez, float lx, float ly, float lz, float ux, float uy, float uz) {
    PBRTState::tMul(Transform::LookAt(P3(ex,ey,ez), P3(lx,ly,lz), V3(ux,uy,uz)));
}
...
void pbrtTransformEnd(void)
{
    PBRTState::transforms.pop();    
}
}
...
std::shared_ptr<const Shape> readply(std::string const& filepath)
{
    ...
    std::shared_ptr<PlyData> vertices, normals, uvs, faces;
    {
        ...
        catch (const std::exception & e) { std::cout << "tinyply exception: " << e.what() << std::endl; }
    ...
    return std::make_shared<Mesh>(std::move(vertexes), std::move(n), std::move(u), std::move(triangles));
}
...
void pbrtObjectEnd() {
    if(PBRTState::objectStack.size() > 0)
    {
        ...
        std::cout << "Made object: " << object.first << " with " << object.second.instances.size() << std::endl;
    }
}
...
void pbrtObjectInstance(std::string const& name) {
    ...
    //std::cout << "Instantiating " << name << " with " << shapes.size() << std::endl;
...
std::shared_ptr<Texture const> makeTexture(std::string const& name, Params const& params, rgba const& defaultValue)
{
...
std::shared_ptr<const Material> makeMaterial(std::string const& type, Params const& params)
{
    ...
    {
        result = std::make_shared<DiffuseMaterial>(makeTexture("Kd", params, rgba(1,0,0,1)));
    }
...
void pbrtTexture(std::string const& name, std::string const& type, std::string const& clas, Params const& params) {
    if(clas == "imagemap")
    {
        auto filename = params.getString("string filename");
    ...
    {
        std::shared_ptr<Texture const> tex1 = makeTexture("tex1", params, rgba(0,1,0,1));
        std::shared_ptr<Texture const> tex2 = makeTexture("tex2", params, rgba(0,1,0,1));
...
void pbrtWorldEnd() {
    auto object = PBRTState::objectStack.top();
    PBRTState::objectStack.pop();
    scene = std::make_shared<Scene>(object.second.instances, object.second.lights);
    std::cout << "Made World with " << object.second.instances.size() << " shapes and " << object.second.lights.size() << " lights" << std::endl;
    pbrtAttributeEnd();
}
...
float parseReal(Tokens& tokens)
{
    return stof(tokens.next());
}

int64_t parseInt(Tokens& tokens)
{
    return stoll(tokens.next());
}
...
Params parseParams(Tokens& tokens)
{
    ...
    while(tokens.peek()[0] == '"')
    {
        ...
        if(param.rfind("string", 0) == 0
        || param.rfind("texture", 0) == 0
...
void parseAttributeBegin(Tokens& tokens)
{
    pbrtAttributeBegin();
}

void parseAttributeEnd(Tokens& tokens)
{
    pbrtAttributeEnd();
}
...
void parseObjectEnd(Tokens& tokens)
{
    pbrtObjectEnd();
}
...
void parseWorldBegin(Tokens& tokens)
{
    pbrtWorldBegin();
}

void parseWorldEnd(Tokens& tokens)
{
    pbrtWorldEnd();
}

void parseIdentity(Tokens& tokens)
{
    pbrtIdentity();
}
...
void parseCoordinateSystem(Tokens& tokens)
{
    parseString(tokens);
}

void parseCoordSysTransform(Tokens& tokens)
{
    parseString(tokens);
}
...
void parseTransformBegin(Tokens& tokens)
{
    pbrtTransformBegin();
}

void parseTransformEnd(Tokens& tokens)
{
    pbrtTransformEnd();
}
...
void readFile(std::string const& path, std::string const& name, std::vector<std::string>& tokens)
{
    ...
    while(std::getline(file, line))
    {
        if(line.rfind("Include", 0) == 0) {
            ...
            if(end != std::string::npos)
                readFile(path, line.substr(start+1,end-(start+1)), tokens);
            else
